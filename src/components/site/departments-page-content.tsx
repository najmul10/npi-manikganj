...
import { ArrowRight, Users, CalendarDays, Building, Briefcase, Sparkles } from "lucide-react";
      ...
      <section className="relative py-16 sm:py-24 lg:py-28 bg-secondary/40">
        <div className="mx-auto max-w-7xl px-4">
          <SectionHeading
            ...
            title={<>All <span className="text-brand">8 Engineering</span> Departments</>}
              ...
              : depts.map((d, idx) => (
                  ...
                  >
                    <div className="relative h-36 sm:h-40 overflow-hidden">
                      <img src={d.image} alt={d.name} className="h-full w-full object-cover group-hover:scale-110 transition-transform duration-700" />
                    ...
                    <div className="p-4 sm:p-5">
                      <h3 className="font-serif font-bold text-lg text-foreground group-hover:text-brand transition-colors">{d.name}</h3>
                      <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed line-clamp-2">{d.tagline}</p>
                      <div className="mt-4 flex items-center justify-between text-xs text-muted-foreground">
                        <span className="inline-flex items-center gap-1.5"><Users className="h-3.5 w-3.5 text-brand" /> {d.seats} seats</span>
                        <span className="inline-flex items-center gap-1.5"><CalendarDays className="h-3.5 w-3.5 text-brand" /> 4 Years</span>
                      </div>
            ...
            <>
              ...
              <div className="px-6 pb-6 space-y-5">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selected.highlights.split(",").map((h, i) => (
                      <Badge key={i} variant="secondary" className="bg-brand/8 text-brand border-brand/15">{h.trim()}</Badge>
                    ))}
                  </div>
                </div>
                <div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-2">
                  <Button onClick={() => { setSelected(null); openAdmission(selected.name); }} className="bg-brand hover:bg-brand-deep text-white flex-1">
...
function Stat({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-xl border border-border bg-secondary/50 p-3 text-center">
      <div className="grid place-items-center h-8 w-8 mx-auto rounded-lg bg-brand/10 text-brand mb-1.5">{icon}</div>
